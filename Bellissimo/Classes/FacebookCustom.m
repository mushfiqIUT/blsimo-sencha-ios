...
//  Universal
//
//  Created by Zakir Hossain on 1/8/13.
//
//
...
#import "FacebookCustom.h"
#import "FBLoginDialogCustom.h"
#import "FBRequestCustom.h"
 ...
 */
- (FBRequestCustom*)openUrl:(NSString *)url
    ...
    [_request release];
    _request = [[FBRequestCustom getRequestWithParams:params
...
- (void)authorizeWithFBAppAuth:(BOOL)tryFBAppAuth
    ...
    if ([device respondsToSelector:@selector(isMultitaskingSupported)] && [device isMultitaskingSupported]) {
        if (tryFBAppAuth) {
            ...
            NSString *fbAppUrl = [FBRequestCustom serializeURL:urlPrefix params:params];
        ...
        if (trySafariAuth && !didOpenOtherApp) {
            ...
            NSString *fbAppUrl = [FBRequestCustom serializeURL:loginDialogURL params:params];
            didOpenOtherApp = [[UIApplication sharedApplication] openURL:[NSURL URLWithString:fbAppUrl]];
        }
    }
    ...
    if (!didOpenOtherApp) {
        [_loginDialog release];
        _loginDialog = [[FBLoginDialogCustom alloc] initWithURL:loginDialogURL
                                              loginParams:params
                                                 delegate:self];
        [_loginDialog show];
    }
 ...
 */
- (FBRequestCustom*)requestWithParams:(NSMutableDictionary *)params
 ...
 *            Returns a pointer to the FBRequest object.
 */
- (FBRequestCustom*)requestWithMethodName:(NSString *)methodName
 ...
 *            Returns a pointer to the FBRequest object.
 */
- (FBRequestCustom*)requestWithGraphPath:(NSString *)graphPath
                             andDelegate:(id <FBRequestDelegate>)delegate {
...
- (void)dialog:(NSString *)action
   ...
   andDelegate:(id <FBDialogDelegate>)delegate {
    ...
    if (action == kLogin) {
        ...
        _fbDialog = [[FBLoginDialogCustom alloc] initWithURL:dialogURL loginParams:params delegate:self];
    } else {
        ...
        _fbDialog = [[FBDialogCustom alloc] initWithURL:dialogURL params:params delegate:delegate];
    }
 ...
 */
- (void)request:(FBRequestCustom*)request didFailWithError:(NSError*)error{
